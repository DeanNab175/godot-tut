...
func handle_request(player_id: int) -> void:
	...
	if _waiting_player == -1:
		...
		NetworkManager.notify_client_status(player_id, "waiting")
	else:
...
func handle_cancel(player_id: int) -> void:
	...
	NetworkManager.notify_client_status(player_id, "cancelled")  # ✅
...
func handle_quit(player_id: int) -> void:
	if not multiplayer.is_server():
		return

	if not _player_rooms.has(player_id):
		print("⚠️ Player %d tried to quit but has no room" % player_id)
		return

	var room_id: int = _player_rooms[player_id]
	print("🚪 Player %d quit room %d" % [player_id, room_id])
	_dissolve_room(room_id, player_id)

	# Confirm back to the quitting player so their UI resets too
	NetworkManager.notify_client_opponent_left(player_id)  # ✅
...
func _create_room(p1: int, p2: int) -> void:
	...
	NetworkManager.notify_client_match_found(p1, room_id)
	NetworkManager.notify_client_match_found(p2, room_id)
...
func _dissolve_room(room_id: int, leaving_player: int) -> void:
	...
	for p in room["players"]:
		...
		if p != leaving_player:
			...
			NetworkManager.notify_client_opponent_left(p)  # ✅

	_rooms.erase(room_id)
	print("🗑️ Room %d dissolved" % room_id)

# ---------------------------------------------------------------
# REMOVED — _notify_client() is no longer needed
# NetworkManager now exposes typed methods directly:
#   NetworkManager.notify_client_match_found(player_id, room_id)
#   NetworkManager.notify_client_status(player_id, status)
#   NetworkManager.notify_client_opponent_left(player_id)
# ---------------------------------------------------------------
